...
tie_breakings = strategies.sampled_from(
    [
        TieBreaking.AWAY_FROM_ZERO,
        TieBreaking.TO_EVEN,
        TieBreaking.TO_ODD,
...
fractions = strategies.builds(Fraction, ints) | strategies.builds(
    Fraction, ints, non_zero_ints
)
fractions |= strategies.builds(Fraction, ints).map(
    lambda value: value + Fraction(1, 2)
)
...
fractions_with_builtins = strategies.builds(
    to_fraction_with_builtin, ints_with_builtins
) | strategies.builds(
    to_fraction_with_builtin, ints_with_builtins, non_zero_ints_with_builtins
)
rationals_with_builtins = fractions_with_builtins | ints_with_builtins
zero_fractions_or_ints_or_builtin_ints = (
    zero_fractions | zero_ints | zero_integers
)
...
small_ints_with_builtins = strategies.builds(
    to_int_with_builtin, small_integers
)
